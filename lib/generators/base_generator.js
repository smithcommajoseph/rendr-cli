...
BaseGenerator.prototype.createDirectory = function(dir, verbose) {
  ...
  try{
    fs.mkdirSync(dir);
    if (!!verbose) this.logToConsole('success', 'generating', 'dir ', dir);
  }
  catch(err) {
...
BaseGenerator.prototype.createFile = function(file, data, verbose) {
  verbose = verbose === false ? verbose : true;
  try{
    fs.writeFileSync(file, data);
    if (!!verbose) this.logToConsole('success', 'generating', 'file', file);
  }
  catch(err){
