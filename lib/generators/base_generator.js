...
BaseGenerator.prototype.getGeneratorArgs = function(args) {
  // first arg is ALWAYS a name for the generator
  return args.slice(1);
};
...
BaseGenerator.prototype.renderCompanions = function(companions) {
  ...
  companions.forEach(function(companion){
    generator = BaseGenerator.getGenerator(companion.generate, companion.args, companion, false);
    generator.generate();
  });
};
...
BaseGenerator.getGenerator = function(cmd, args, opts, hasCompanions) {
  var generatorName = BaseGenerator.getGeneratorName(cmd),
...
BaseGenerator.getGeneratorName = function(cmd) {
  var generatorName = ('new' == cmd) ? 'app' : cmd;
